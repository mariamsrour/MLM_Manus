    ...
    public virtual async Task<IList<string>> RefundAsync(Order order)
    {
        ...
        try
        {
            ...
            if (result.Success)
            {
                ...
                await _orderService.UpdateOrderAsync(order);
                // add to wallet 
                var orderItemsIds = (await _orderService.GetOrderItemsAsync(order.Id)).Select(x => x.Id).ToList();
                var returnrequests = (await _returnRequestService.SearchReturnRequestsAsync(order.StoreId, order.CustomerId))
                    .Where(x => orderItemsIds.Contains(x.OrderItemId));
                var returntoWalletAction = (await _returnRequestService.GetAllReturnRequestActionsAsync()).Where(x => x.Name.ToLower().Contains("wallet")).FirstOrDefault();
                if (returnrequests.Any(x => x.RequestedAction == returntoWalletAction.Name))
                {
                    // refund to wallet
                    await _walletService.AddTransactionAsync(order.CustomerId, totalAmountRefunded, "Refund", "Refund request added to wallet for order no." + order.Id.ToString(), order.CustomerCurrencyCode);
                }
                //add a note
