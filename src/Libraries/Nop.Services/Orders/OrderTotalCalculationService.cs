...
public partial class OrderTotalCalculationService : IOrderTotalCalculationService
{
    ...
    GetShoppingCartShippingTotalAsync(IList<ShoppingCartItem> cart, bool includingTax)
    {
        ...
        // 1. Check for free shipping first (keep existing logic)
        var isFreeShipping = await IsFreeShippingAsync(cart);
        if (isFreeShipping)
            return (decimal.Zero, taxRate, appliedDiscounts);

        // 2. Initialize shipping total
        decimal totalShipping = 0m;

        // 3. Iterate through each cart item to calculate shipping based on SelectedShippingMethod
        foreach (var cartItem in cart)
        {
            if (cartItem.SelectedShippingMethod == 0)
            {
                // Handle missing shipping method (e.g., log error, skip item, or throw exception)
                // throw new NopException($"No shipping method selected for product ID {cartItem.ProductId}");
                totalShipping += 0;
            }

            var method = await _shippingService.GetShippingMethodByIdAsync(cartItem.SelectedShippingMethod);
          
            var address = await _addressService.GetAddressByIdAsync(customer.ShippingAddressId ?? customer.BillingAddressId ?? 0);
            if (address == null)
            {
                address = new Address
                {
            ...
            var result = await _shippingService.GetShippingOptionsAsync(cart, address, customer, storeId: store.Id); //await PrepareEstimateShippingResultModelAsync(new[] { sci }, productEstimateShipping, false);
            var selectedOption = result.ShippingOptions.FirstOrDefault(x => x.Name == method.Name);


            // 7. Adjust rate with discounts and add to total
            var (adjustedRate, itemDiscounts) = await AdjustShippingRateAsync(selectedOption.Rate, new List<ShoppingCartItem> { cartItem });
            totalShipping += adjustedRate;
            appliedDiscounts.AddRange(itemDiscounts);
        }
        ...
        // 8. Apply tax to the total shipping
        (var shippingTotalTaxed, taxRate) = await _taxService.GetShippingPriceAsync(
            totalShipping,
            includingTax,
            customer
        );

        // 9. Round the final total if needed
        if (_shoppingCartSettings.RoundPricesDuringCalculation)
            shippingTotalTaxed = await _priceCalculationService.RoundPriceAsync(shippingTotalTaxed);
