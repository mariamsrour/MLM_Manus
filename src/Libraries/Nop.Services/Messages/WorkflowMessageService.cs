...
public partial class WorkflowMessageService : IWorkflowMessageService
{
    ...
    protected readonly IEmailVerificationCodeService _emailVerificationCodeService;
    ...
    public virtual async Task<IList<int>> SendCustomerEmailValidationMessageAsync(Customer customer, int languageId)
    {
        ...
        if (!messageTemplates.Any())
            return new List<int>();
        var code = await _emailVerificationCodeService.GetLatestCodeAsync(customer.Id);

        //tokens
        var commonTokens = new List<Token>();
        await _messageTokenProvider.AddCustomerTokensAsync(commonTokens, customer, code.Code);
    ...
    #region newCustomEmails 
    public virtual async Task<IList<int>> SendAdStatusChangedNotificationAsync(Customer customer, Product ad, int languageId)
    {
        var store = await _storeContext.GetCurrentStoreAsync();
        languageId = await EnsureLanguageIsActiveAsync(languageId, store.Id);

        var templates = ad.AdStatus == AdStatus.Active ? await GetActiveMessageTemplatesAsync("AD.ACCEPTED", store.Id): await GetActiveMessageTemplatesAsync(" AD.REJECTED", store.Id);
        ...
        tokens.Add(new Token("Ad.Id", ad.Id));
        var adSeName = await _urlRecordService.GetSeNameAsync(ad);
        return await templates.SelectAwait(async t =>
        {
            var emailAccount = await GetEmailAccountOfMessageTemplateAsync(t, languageId);
            await _messageTokenProvider.AddStoreTokensAsync(tokens, store, emailAccount);
            tokens.Add(new Token("Ad.Url", store.Url + adSeName));

            await _eventPublisher.MessageTokensAddedAsync(t, tokens);

            var name = await _customerService.GetCustomerFullNameAsync(customer);
            return await SendNotificationAsync(t, emailAccount, languageId, tokens, customer.Email, name);
        }).ToListAsync();
    }

    public virtual async Task<IList<int>> SendSubscriptionInvoiceCustomerNotificationAsync(
    Customer customer, UserSubscription subscription, int languageId, byte[] pdfBytes)
    {
        var store = await _storeContext.GetCurrentStoreAsync();
        languageId = await EnsureLanguageIsActiveAsync(languageId, store.Id);

        var templates = await GetActiveMessageTemplatesAsync("SUBSCRIPTION.INVOICE", store.Id);
        if (!templates.Any())
            return new List<int>();

        var tokens = new List<Token>();
        await _messageTokenProvider.AddCustomerTokensAsync(tokens, customer);
        tokens.Add(new Token("Subscription.Id", subscription.Id));
        tokens.Add(new Token("Subscription.Amount", subscription.PaidPrice.ToString("C")));
        ...
        return await templates.SelectAwait(async t =>
        {
            ...
            var filePath = Path.Combine(CommonHelper.DefaultFileProvider.MapPath("~/App_Data/Downloads"),
                                        $"Invoice_{subscription.Id}.pdf");

            await File.WriteAllBytesAsync(filePath, pdfBytes);

            var name = await _customerService.GetCustomerFullNameAsync(customer);
            return await SendNotificationAsync(t, emailAccount, languageId, tokens,
                customer.Email, name, filePath, $"Invoice_{subscription.Id}.pdf");
        }).ToListAsync();
    }
