...
public partial class ProductService : IProductService
{
    ...
    //    var productsQuery = _productRepository.Table.AsNoTracking();
    ...
    //        {
    //            productsByKeywords =
    //                from p in _productRepository.Table.AsNoTracking()
    ...
    //                    from lp in _localizedPropertyRepository.Table.AsNoTracking()
    ...
    //            productsByKeywords = productsByKeywords.Union(
    //                from pc in _productCategoryRepository.Table.AsNoTracking()
    ...
    //                productsByKeywords = productsByKeywords.Union(
    //                    from pc in _productCategoryRepository.Table.AsNoTracking()
    //                    join lp in _localizedPropertyRepository.Table.AsNoTracking() on pc.CategoryId equals lp.EntityId
    ...
    //                from psa in _productSpecificationAttributeRepository.Table.AsNoTracking()
...
)
    {
        ...
        productsQuery = productsQuery.Where(p =>
         ...
         (vendorId == 0 || p.VendorId == vendorId) &&
         (productType == null || p.ProductTypeId == (int)productType) &&
         (showHidden || (DateTime.UtcNow >= (p.AvailableStartDateTimeUtc ?? DateTime.MinValue)
        ...
        // Keyword filtering (simple fallback, no plugin search, no localized, no tags)
        if (!string.IsNullOrEmpty(keywords))
        {
            productsQuery = productsQuery.Where(p =>
        ...
        {
            productsQuery = productsQuery.Where(p => p.County == country);
        }

        if (!string.IsNullOrEmpty(city))
        {
            productsQuery = productsQuery.Where(p => p.City == city);
        }
        ...
        if (categoryIds?.Any() == true)
        {
            ...
            productsQuery = productsQuery.Where(p => productCategoryQuery.Contains(p.Id));
        }

        // Specification filtering
        if (filteredSpecOptions?.Any() == true)
        {
            var specGroups = filteredSpecOptions.GroupBy(o => o.SpecificationAttributeId);
            foreach (var group in specGroups)
            {
                var optionIds = group.Select(o => o.Id).ToList();
                var psaQuery = _productSpecificationAttributeRepository.Table
                    .Where(psa => psa.AllowFiltering && optionIds.Contains(psa.SpecificationAttributeOptionId))
                    .Select(psa => psa.ProductId);

                productsQuery = productsQuery.Where(p => psaQuery.Contains(p.Id));
            }
        }
        ...
        {
            var orderedQuery = productsQuery.OrderByDescending(x=>x.UpdatedOnUtc);

            return await orderedQuery.ToPagedListAsync(pageIndex, pageSize);
        } else
        {
            var orderedQuery = productsQuery.OrderBy(_localizedPropertyRepository, language, orderBy);
    ...
    public virtual async Task<Product> GetProductBySkuAsync(string sku)
    {
        ...
        var query = from p in _productRepository.Table
            ...
            where !p.Deleted &&
                  p.Sku == sku
            select p;
        var product = await Microsoft.EntityFrameworkCore
                     .EntityFrameworkQueryableExtensions
                     .FirstOrDefaultAsync(query);  //await query.FirstOrDefaultAsync();
