...
public partial class ForumService : IForumService
{
    ...
    //public virtual async Task<IList<PrivateMessage>> GetInboxGroupedByAdAndSenderAsync(int toCustomerId, int storeId = 0)
    //{
    ...
    //   // query = query.Where(pm => (pm.ToCustomerId == toCustomerId  || pm.FromCustomerId == toCustomerId ) && (!pm.IsDeletedByRecipient));

    //   // // Group by AdId and FromCustomerId
    //   // //var groupedMessages = await query
    //   // //    .GroupBy(pm => new { pm.AdId, pm.FromCustomerId })
    //   // //    .Select(g => g.OrderByDescending(m => m.CreatedOnUtc).FirstOrDefault()) // get latest message per group
    //   // //    .OrderByDescending(m => m.CreatedOnUtc) // order final list by date
    //   // //    .ToListAsync();
    ...
    public virtual async Task<IList<PrivateMessage>> GetInboxGroupedByAdAndSenderAsync(int toCustomerId, int storeId = 0)
    {
        // Step 1: Start the base query
        //var query = _forumPrivateMessageRepository.Table
        //    .Where(pm => (pm.ToCustomerId == toCustomerId || pm.FromCustomerId == toCustomerId)
        //                 && !pm.IsDeletedByRecipient);
        ...
        //// Step 2: Get all messages from DB first (EF Core can't do double grouping easily)
        //var allMessages = await query.ToListAsync();

        //// Step 3: Get latest message per (AdId, FromCustomerId)
        //var latestPerAdAndSender = allMessages
        //    .GroupBy(pm => new { pm.AdId, pm.FromCustomerId })
        //    .Select(g => g.OrderByDescending(m => m.CreatedOnUtc).First())
        //   .DistinctBy(x => new { x.AdId, x.FromCustomerId, x.ToCustomerId })
        //   .ToList();

        //// Step 4: From the above, get the latest message per AdId
        //var finalGrouped = latestPerAdAndSender
        //    .GroupBy(pm => pm.AdId)
        //    .Select(g => g.OrderByDescending(m => m.CreatedOnUtc).First())
        //    .OrderByDescending(m => m.CreatedOnUtc)
        //    .ToList();

        //return latestPerAdAndSender.OrderByDescending(m => m.CreatedOnUtc).ToList();

        //Normalize keys inside the query so SQL handles it
       var query = _forumPrivateMessageRepository.Table
           .Where(pm => (pm.ToCustomerId == toCustomerId || pm.FromCustomerId == toCustomerId)
                        && !pm.IsDeletedByRecipient);
        ...
        var groupedThreads = await query
    .GroupBy(pm => new
    {
        pm.AdId,
        Customer1 = pm.FromCustomerId < pm.ToCustomerId ? pm.FromCustomerId : pm.ToCustomerId,
        Customer2 = pm.FromCustomerId < pm.ToCustomerId ? pm.ToCustomerId : pm.FromCustomerId
    })
    .Select(g => g
        .OrderByDescending(m => m.CreatedOnUtc)
        .FirstOrDefault())
    .ToListAsync(); // SQL runs here
    ...
    public virtual async Task<IList<PrivateMessage>> GetInboxGroupedByAdAndReceiverAsync(int toCustomerId, int storeId = 0)
    {
        var query = _forumPrivateMessageRepository.Table;
        ...
        query = query.Where(pm => pm.FromCustomerId == toCustomerId && !pm.IsDeletedByAuthor);

        //// Group by AdId and FromCustomerId
        //var groupedMessages = await query
        //    .GroupBy(pm => new { pm.AdId, pm.ToCustomerId })
        //    .Select(g => g.OrderByDescending(m => m.CreatedOnUtc).FirstOrDefault()) // get latest message per group
        //    .OrderByDescending(m => m.CreatedOnUtc) // order final list by date
        //    .ToListAsync();
        var allMessages = await query.ToListAsync();
    ...
    public virtual async Task<IList<PrivateMessage>> GetConversationForAdAsync(int customerId1, int customerId2, int adId, int storeId = 0)
    {
        var query = _forumPrivateMessageRepository.Table;
        ...
        query = query.Where(pm => pm.AdId == adId &&
            (
                (pm.FromCustomerId == customerId1 && pm.ToCustomerId == customerId2 && !pm.IsDeletedByAuthor)
                ||
                (pm.FromCustomerId == customerId2 && pm.ToCustomerId == customerId1 && !pm.IsDeletedByRecipient)
            ));

        query = query.OrderBy(pm => pm.CreatedOnUtc);

        var result = await query.ToListAsync();
        //foreach (var item in result)
        //{
        //    item.IsRead = true;
        //    await _forumPrivateMessageRepository.UpdateAsync(item);
        //}
   ...
   public async Task<int> GetUnreadMessagesCount(int toCustomerId)
    {
        var query = _forumPrivateMessageRepository.Table
         .Where(pm => (pm.ToCustomerId == toCustomerId || pm.FromCustomerId == toCustomerId)
                      && !pm.IsDeletedByRecipient);
    ...
    .ToListAsync(); // SQL runs here
        ...
        return groupedThreads.Where(x => x.ToCustomerId == toCustomerId && !x.IsRead).Count();
