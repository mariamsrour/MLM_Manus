...
public partial interface IForumService
{
    ...
    Task<IList<PrivateMessage>> GetInboxGroupedByAdAndSenderAsync(int toCustomerId, int storeId = 0);

    Task<IList<PrivateMessage>> GetInboxGroupedByAdAndReceiverAsync(int toCustomerId, int storeId = 0);

    Task<IList<PrivateMessage>> GetConversationForAdAsync(int customerId1, int customerId2, int adId, int storeId = 0);

    Task<int> GetUnreadMessagesCount(int customerId);
}