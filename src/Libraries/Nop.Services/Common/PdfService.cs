...
public partial class PdfService : IPdfService
{
    ...
    public virtual async Task PrintSubscriptionInvoiceToPdfAsync(Stream stream, UserSubscription subscription)
    {
        if (stream == null)
        ...
        var cus = await _customerService.GetCustomerByIdAsync(subscription.CustomerId);
        // QuestPDF document
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                {
                    col.Item().Text($"Invoice ID: {subscription.Id}").FontSize(12);
                    col.Item().Text($"Customer: {(cus != null ? cus.Email : "")}").FontSize(12);
                    col.Item().Text($"Date: {subscription.StartDate:yyyy-MM-dd HH:mm}").FontSize(12);
                    col.Item().Text($"Amount: {subscription.PaidPrice:C}").FontSize(12);
                    col.Item().Text("");
                    col.Item().Text("Thank you for your subscription!").FontSize(12);
                });
                ...
                page.Footer()
                    ...
                    .Text(text =>
                    {
                        ...
                        text.Span(DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm")).FontSize(10);
                    });
            });
        });

        // Generate PDF into the provided stream
