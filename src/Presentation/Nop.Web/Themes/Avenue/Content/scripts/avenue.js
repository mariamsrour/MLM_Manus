(function ($) {
    $(document).ready(function () {
      ...
      // phone
      var phoneInputs = document.querySelectorAll(".phoneInputClass");
      ...
      phoneInputs.forEach(function (phoneInput) {
        var iti = window.intlTelInput(phoneInput, {
          ...
          preferredCountries: ["us", "gb", "sa", "eg"],
        });

        // Listen for country change to update the dial code
        phoneInput.addEventListener('countrychange', function () {
          var dialCode = iti.getSelectedCountryData().dialCode;
          phoneInput.value = dialCode + phoneInput.value.replace(/^(\+?\d*)/, '');
        });
        // Copy the full number into the original asp-for="Phone" input for form submission
        //phoneInput.addEventListener("blur", function () {
        //  var fullPhoneNumber = iti.getNumber(); // Get full number with country code
        //  phoneInput.value = fullPhoneNumber; // Override value in the bound field
        //});
      });
