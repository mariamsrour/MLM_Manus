...
public partial class ReturnRequestController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> ReturnRequestSubmit(int orderId, SubmitReturnRequestModel model, IFormCollection form)
    {
        ...
        foreach (var orderItem in orderItems)
        {
            ...
            {
                var download = await _downloadService.GetDownloadByGuidAsync(model.Items.FirstOrDefault(x => x.Id == orderItem.Id).UploadedFileGuid);
                if (download != null)
            ...
            if (quantity > 0)
            {
                var rrr = await _returnRequestService.GetReturnRequestReasonByIdAsync(model.Items.FirstOrDefault(x=>x.Id == orderItem.Id).ReturnRequestReasonId);
                var rra = await _returnRequestService.GetReturnRequestActionByIdAsync(model.Items.FirstOrDefault(x => x.Id == orderItem.Id).ReturnRequestActionId);
                ...
                var rr = new ReturnRequest
                {
                    ...
                    CustomerComments = model.Items.FirstOrDefault(x => x.Id == orderItem.Id).Comments,
