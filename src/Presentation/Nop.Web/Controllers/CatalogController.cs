...
public partial class CatalogController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> Vendor( CatalogProductsCommand command, int vendorId = 0)
    {
        if(vendorId == 0)
            vendorId = (await _workContext.GetCurrentCustomerAsync()).VendorId;
    ...
    public virtual async Task<IActionResult> MyAds(CatalogProductsCommand command)
    {
        ...
        if(! (command.OrderBy > 0))
        {
            ...
            model.CatalogProductsModel.Products = model.CatalogProductsModel.Products.OrderByDescending(x => x.Id)
                .ToList();
        }
    ...
    public virtual async Task<IActionResult> MyReviews(CatalogProductsCommand command)
    {
        if (!await _customerService.IsRegisteredAsync(await _workContext.GetCurrentCustomerAsync()))
            return Challenge();
    ...
    [CheckLanguageSeoCode(ignore: true)]
    public virtual async Task<IActionResult> GetAdsProducts(int vendorId, CatalogProductsCommand command)
    {
        var vendor = await _vendorService.GetVendorByIdAsync(vendorId);
        ...
        return PartialView("_myAdsProductsList", model);
    }
