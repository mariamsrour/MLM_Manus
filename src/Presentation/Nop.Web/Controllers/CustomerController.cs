...
public partial class CustomerController : BasePublicController
{
    ...
    protected readonly INewsItemReadService _newsItemReadService;
    ...
    public CustomerController(AddressSettings addressSettings,
        ...
        INewsItemReadService newsItemReadService)
    {
        ...
        _newsItemReadService = newsItemReadService;
    }
            ...
            if (registrationResult.Success)
            {
                ...
                switch (_customerSettings.UserRegistrationType)
                {
                    case UserRegistrationType.EmailValidation:
                        ...
                        await _emailVerificationCodeService.InsertAsync(new EmailVerificationCode
                        {
                        ...
                        //result
                        return RedirectToRoute("VerifyEmail", new { customerId = customer.Id});
    ...
    [HttpGet]
    public IActionResult VerifyEmail(int customerId)
    {
    ...
    public async Task<IActionResult> VerifyEmail(VerifyEmailModel model)
    {
        ...
        var codeEntry = await _emailVerificationCodeService.GetLatestCodeAsync(customer.Id);
        if (codeEntry == null || codeEntry.Code != model.Code)
        {
            ModelState.AddModelError("", "Invalid or expired code.");
            return View(model);
        }

        //if ((DateTime.UtcNow - codeEntry.CreatedOnUtc).TotalMinutes > 10)
        //{
        //    ModelState.AddModelError("", "Code has expired.");
        //    return View(model);
        //}
        await _genericAttributeService.SaveAttributeAsync(customer, "IsEmailVerified", true);

        // ✅ Optional: delete or mark OTP as used
        await _emailVerificationCodeService.DeleteAsync(codeEntry); // OR mark as used
        var currentLanguage = await _workContext.GetWorkingLanguageAsync();
        await _workflowMessageService.SendCustomerWelcomeMessageAsync(customer, currentLanguage.Id);
        //raise event       
        await _eventPublisher.PublishAsync(new CustomerActivatedEvent(customer));

        var returnUrl = Url.RouteUrl("RegisterResult", new { resultId = ((int)UserRegistrationType.Standard).ToString(), returnUrl = Url.RouteUrl("Homepage") });
        return await _customerRegistrationService.SignInCustomerAsync(customer, returnUrl, true);
    }
    ...
    public virtual async Task<IActionResult> Notifications(NewsPagingFilteringModel command)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        if (!await _customerService.IsRegisteredAsync(customer))
            return Challenge();
        ...
        var allIds = model.NewsItems.Select(x => x.Id);
        await _newsItemReadService.MarkAllAsReadAsync(customer.Id, allIds);
    ...
    public virtual async Task<IActionResult> Info()
    {
        ...
        var myavatar = new CustomerAvatarModel();
        ViewBag.avatar = await _customerModelFactory.PrepareCustomerAvatarModelAsync(myavatar);
    ...
    public virtual async Task<IActionResult> Info(CustomerInfoModel model, IFormCollection form)
    {
        ...
        try
        {
            if (ModelState.IsValid)
            {
                ...
                if (vendor != null)
                {
                    vendor.Name = $"{customer.FirstName} {customer.LastName}";
