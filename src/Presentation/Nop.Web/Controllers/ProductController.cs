...
public partial class ProductController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> ProductDetails(int productId, int updatecartitemid = 0)
    {
        ...
        var availableshipping = await _shippingService.GetAllShippingMethodsAsync();
        ...
        var result = await _shoppingCartModelFactory.PrepareEstimateShippingResultModelAsync(new[] { wrappedProduct }, model.ProductEstimateShipping, false);
        var daysTxt = string.Format(await _localizationService.GetResourceAsync("Product.BusinessDays"));
        model.AvailableShippingMethods = result.ShippingOptions.Select(sm => new SelectListItem
        {
            Text = $"{sm.Name} - {sm.TransitDays} {daysTxt} ({sm.Price})",
            Value = availableshipping.FirstOrDefault(x => x.Name == sm.Name).Id.ToString()
        }).ToList();
        //template
