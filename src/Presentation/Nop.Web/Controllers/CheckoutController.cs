...
public partial class CheckoutController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> ConfirmOrder(bool captchaValid)
    {
        ...
        try
        {
            ...
            var walletAppliedAmount = await _genericAttributeService.GetAttributeAsync<decimal?>(customer, "CheckoutWalletAppliedAmount", store.Id);

            if (walletAppliedAmount.HasValue)
            {
                // Adjust the total
                processPaymentRequest.OrderTotal -= walletAppliedAmount.Value;

                // Clear the temporary wallet adjustment
                await _genericAttributeService.SaveAttributeAsync<decimal?>(customer, "CheckoutWalletAppliedAmount", null, store.Id);
            }
            //// Wallet adjustment
            //var adjustedTotal = await _genericAttributeService.GetAttributeAsync<decimal?>(customer, "CheckoutWalletAdjustedTotal", store.Id);
            //if (adjustedTotal.HasValue)
            //{
            //    processPaymentRequest.OrderTotal = adjustedTotal.Value;

            //    // Ensure wallet deductions are applied only once
            //    await _genericAttributeService.SaveAttributeAsync<decimal?>(customer, "CheckoutWalletAdjustedTotal", null, store.Id);
            //}
            ...
            if (placeOrderResult.Success)
            {
                ...
                // Deduct wallet balance only if the order is successfully placed
                if (walletAppliedAmount.HasValue)
                {
                    var curency = await _workContext.GetWorkingCurrencyAsync();
                    await _walletService.AddTransactionAsync(customer.Id, -walletAppliedAmount.Value, "Debit", "Order Purchase no." + placeOrderResult.PlacedOrder.Id.ToString(), curency.CurrencyCode);
                }
    ...
    protected virtual async Task<JsonResult> OpcLoadStepAfterShippingMethod(IList<ShoppingCartItem> cart)
    {
        ...
        if (isPaymentWorkflowRequired)
        {
                ...
                paymentMethodModel.PaymentMethods.Count == 1 && !paymentMethodModel.DisplayRewardPoints)
            {
                ...
                return await OpcLoadStepAfterPaymentMethod(paymentMethodInst, cart, false);
            }
    ...
    protected virtual async Task<JsonResult> OpcLoadStepAfterPaymentMethod(IPaymentMethod paymentMethod, IList<ShoppingCartItem> cart, bool useWallet)
    {
        if (paymentMethod.SkipPaymentInfo ||
            (paymentMethod.PaymentMethodType == PaymentMethodType.Redirection && _paymentSettings.SkipPaymentInfoStepForRedirectionPaymentMethods))
        {
            ...
            var confirmOrderModel = await _checkoutModelFactory.PrepareConfirmOrderModelWithWalletAsync(cart, useWallet);
            return Json(new
            {
                update_section = new UpdateSectionJsonModel
                {
    ...
    [HttpPost]
    public virtual async Task<IActionResult> OpcSavePaymentMethod(string paymentmethod, CheckoutPaymentMethodModel model, bool useWallet)
    {
        try
        {
            //validation
            ...
            if (useWallet)
            {
                ...
                var walletBalance = await _walletService.GetWalletBalanceAsync(customer.Id, "USD");

                // Calculate cart total
                var cartTotalResult = await _orderTotalCalculationService.GetShoppingCartTotalAsync(cart, true);
                var cartTotal = cartTotalResult.shoppingCartTotal ?? 0;

                // Determine wallet deduction
                var walletApplied = Math.Min(walletBalance, cartTotal);

                // Store adjusted total for later use

                // Deduct wallet balance immediately
                if (walletApplied > 0)
                {
                    await _genericAttributeService.SaveAttributeAsync(customer, "CheckoutWalletAdjustedTotal", cartTotal - walletApplied, store.Id);

                    await _genericAttributeService.SaveAttributeAsync(customer, "CheckoutWalletAppliedAmount", walletApplied, store.Id);
                }
            }

            // Save the useWallet flag as a generic attribute
            await _genericAttributeService.SaveAttributeAsync(customer, "UseWallet", useWallet, store.Id);
            ...
            if (!_paymentPluginManager.IsPluginActive(paymentMethodInst) && paymentmethod != "Wallet")
                throw new Exception("Selected payment method can't be parsed");
            ...
            return await OpcLoadStepAfterPaymentMethod(paymentMethodInst, cart, useWallet);
        }
        catch (Exception exc)
        {
            await _logger.WarningAsync(exc.Message, exc, await _workContext.GetCurrentCustomerAsync());
    ...
    [HttpPost]
    public async Task<IActionResult> ProcessWalletPayment(int orderId, decimal orderTotal, string currencyCode)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var walletBalance = await _walletService.GetWalletBalanceAsync(customer.Id, currencyCode);
        ...
        await _walletService.AddTransactionAsync(customer.Id, -orderTotal, "Debit", $"Payment for Order #{orderId}", currencyCode);
