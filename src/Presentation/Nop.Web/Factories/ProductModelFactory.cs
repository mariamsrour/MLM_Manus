...
public partial class ProductModelFactory : IProductModelFactory
{
    ...
    public virtual async Task<List<ProductSpecificationAttributeModel>> PrepareProductSpecificationAttributeListAsync(int groupId)
    {
        //get product specification attributes
        var productSpecificationAttributes = await _specificationAttributeService.GetSpecificationAttributesByGroupIdAsync(specificationAttributeGroupId: groupId);

        var list = new List<ProductSpecificationAttributeModel>();
        ...
        foreach (var attribute in productSpecificationAttributes)
        {
            ...
            var specoptions = await _specificationAttributeService.GetSpecificationAttributeOptionsBySpecificationAttributeAsync(attribute.Id);
            productSpecificationAttributeModel.Name = attribute.Name;
            productSpecificationAttributeModel.Id = attribute.Id;


            foreach (var specAttributeOption in specoptions)
            {
                ...
                var value = new ProductSpecificationAttributeValueModel
                {
                    ...
                    AttributeOptionId = specAttributeOption.Id,
                    ValueRaw = WebUtility.HtmlEncode(await _localizationService.GetLocalizedAsync(specAttributeOption, x => x.Name)),
                    Icon = specAttributeOption.Icon
                };
                productSpecificationAttributeModel.Values.Add(value);

            }
            list.Add(productSpecificationAttributeModel);
    ...
    /// </returns>
    public virtual async Task<IList<PictureModel>> PrepareProductOverviewPicturesModelAsync(Product product, int? productThumbPictureSize = null)
    {
        ArgumentNullException.ThrowIfNull(product);
    ...
    /// </returns>
    protected virtual async Task<IList<ProductDetailsModel.ProductAttributeModel>> PrepareProductAttributeModelsAsync(Product product, ShoppingCartItem updatecartitem = null)
    {
        ArgumentNullException.ThrowIfNull(product);
        ...
        bool forceRedirectionAfterAddingToCart = false)
    {
        ...
        foreach (var product in products)
        {
            ...
            //attributes
            model.ProductAttributes = await PrepareProductAttributeModelsAsync(product);
            ...
            var categoryId = (await _categoryService.GetProductCategoriesByProductIdAsync(product.Id)).FirstOrDefault()?.CategoryId ?? 0;
            if(categoryId > 0)
            {
                model.Category = (await _categoryService.GetCategoryByIdAsync(categoryId)).Name;
            }
