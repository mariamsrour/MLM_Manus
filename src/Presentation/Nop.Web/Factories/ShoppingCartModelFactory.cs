...
public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
{
    ...
    protected virtual async Task<ShoppingCartModel.ShoppingCartItemModel> PrepareShoppingCartItemModelAsync(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
    {
        ...
        var method = await _shippingService.GetShippingMethodByIdAsync(sci.SelectedShippingMethod);
        var productEstimateShipping = new ProductDetailsModel.ProductEstimateShippingModel();
        productEstimateShipping.ProductId = sci.ProductId;
        var result = await PrepareEstimateShippingResultModelAsync(new[] { sci }, productEstimateShipping, false);
        var daysTxt = string.Format(await _localizationService.GetResourceAsync("Product.BusinessDays"));
        var sm = result.ShippingOptions.FirstOrDefault(x => x.Name == method.Name);
        var shipping_txt = $"{sm.Name} - {sm.TransitDays} {daysTxt} ({sm.Price})";
        ...
        var cartItemModel = new ShoppingCartModel.ShoppingCartItemModel
        {
            ...
            Attributes = sci.AttributesXml,
            selectedShippingMethodID = sci.SelectedShippingMethod,
            selectedShippingMethod = shipping_txt
    ...
    protected virtual async Task<WishlistModel.ShoppingCartItemModel> PrepareWishlistItemModelAsync(ShoppingCartItem sci)
    {
        ...
        var cartItemModel = new WishlistModel.ShoppingCartItemModel
        {
            ...
            Quantity = sci.Quantity,
            AttributeInfo = await _productAttributeFormatter.FormatAttributesAsync(product, sci.AttributesXml),
            DateAdded = await _dateTimeHelper.ConvertToUserTimeAsync(sci.CreatedOnUtc, DateTimeKind.Local),
            DatePosted = await _dateTimeHelper.ConvertToUserTimeAsync(product.CreatedOnUtc, DateTimeKind.Local),
        };
            ...
            if (rawShippingOptions.Any())
            {
                foreach (var option in rawShippingOptions)
                {
                    ...
                    model.ShippingOptions.Add(new EstimateShippingResultModel.ShippingOptionModel
                    {
                        ...
                        Selected = selected,
                        TransitDays = option.TransitDays ?? 0
                    });
                }
