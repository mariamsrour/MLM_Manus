...
public partial class PrivateMessagesModelFactory : IPrivateMessagesModelFactory
{
    ...
    public virtual async Task<PrivateMessageListModel> PrepareInboxModelAsync(int page, string tab)
    {
        ...
        var pagerModel = new PagerModel(_localizationService)
        {
            ...
            RouteActionName = "CustomerMessages",
            UseRouteLinks = true,
    ...
    public virtual async Task<PrivateMessageListModel> PrepareInboxModelAsync()
    {
        ...
        var store = await _storeContext.GetCurrentStoreAsync();
        var customer = await _workContext.GetCurrentCustomerAsync();
        var list = await _forumService.GetInboxGroupedByAdAndSenderAsync(customer.Id, store.Id);
    ...
    public virtual async Task<PrivateMessageListModel> PrepareSentModelAsync()
    {
        ...
        var store = await _storeContext.GetCurrentStoreAsync();
        var customer = await _workContext.GetCurrentCustomerAsync();
        var list = await _forumService.GetInboxGroupedByAdAndReceiverAsync(customer.Id, store.Id);
        foreach (var pm in list)
            messages.Add(await PreparePrivateMessageModelAsync(pm));
    ...
    public virtual async Task<PrivateMessageModel> PreparePrivateMessageModelAsync(PrivateMessage pm)
    {
        ...
        var adPictureUrl = await _pictureService.GetPictureUrlAsync(pm.PictureId);
        ...
        var model = new PrivateMessageModel
        {
            ...
            //(await _dateTimeHelper.ConvertToUserTimeAsync(pm.CreatedOnUtc, await _workContext.GetCurrentCustomerAsync())).RelativeFormat()

            CreatedOn = await _dateTimeHelper.ConvertToUserTimeAsync(pm.CreatedOnUtc, DateTimeKind.Local),
            IsRead = pm.IsRead,
            AdId = pm.AdId,
            AdPictureId = pm.PictureId,
