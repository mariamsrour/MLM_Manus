...
public partial class CatalogModelFactory : ICatalogModelFactory
{
    ...
    public virtual async Task<CategoryNavigationModel> PrepareCategoryNavigationModelAsync(int currentCategoryId, int currentProductId)
    {
        ...
        var currentCategory = await _categoryService.GetCategoryByIdAsync(currentCategoryId);
        if (currentCategory != null)
        {
            model.CurrentCategoryId = currentCategory.Id;

            if (currentCategory.ParentCategoryId > 0)
            {
                var parentCategory = await _categoryService.GetCategoryByIdAsync(currentCategory.ParentCategoryId);
                if (parentCategory != null)
                {
                    if(parentCategory.ParentCategoryId > 0)
                    {
                        //if parent category has its own parent
                        model.CurrentDepartmentId = (await _categoryService.GetCategoryByIdAsync(parentCategory.ParentCategoryId)).Id;
                        model.CurrentCategoryId = parentCategory.Id;
                        model.CurrentSubCategoryId = currentCategory.Id;

                    }
                    else
                    {
                        model.CurrentDepartmentId = parentCategory.Id;
                        model.CurrentSubCategoryId = 0;
                    }
            ...
            {
                model.CurrentDepartmentId = currentCategory.Id;
            }
        } else
        {
            model.CurrentCategoryId = activeCategoryId;
            model.CurrentDepartmentId = 31;
            model.CurrentSubCategoryId = 0;
        }
    ...
    public virtual async Task<CatalogProductsModel> PrepareCategoryProductsModelAsync(Category category, CatalogProductsCommand command)
    {
        ...
        //city filter
        await PrepareLocationOptionsAsync(model, command);
        ...
        var products = await _productService.SearchProductsAsync(
            ...
            productType: command.ProductType.HasValue? (ProductType)command.ProductType:null , // <-- add this line
    ...
    public virtual async Task<CatalogProductsModel> PrepareVendorProductsModelAsync(Vendor vendor, CatalogProductsCommand command)
    {
        ...
        if (_catalogSettings.EnablePriceRangeFiltering && vendor.PriceRangeFiltering)
        {
            ...
            if (!vendor.ManuallyPriceRange)
            {
                ...
                {
                    var products = await _productService.SearchProductsAsync(0, 1,
                        showHidden: command.All.HasValue,
                        vendorId: vendor.Id,
                        storeId: store.Id,
                        visibleIndividuallyOnly: true,
                        orderBy: orderBy);
    ...
    public virtual async Task PrepareLocationOptionsAsync(CatalogProductsModel model, CatalogProductsCommand command)
    {
        ...
        model.AvailableCities.Insert(0, new SelectListItem()
        {
            Text = "City",
            Value = ""
        });
