...
public partial class OrderModelFactory : IOrderModelFactory
{
    ...
    public async Task<VendorOrderReportListModel> BestSellingVendors(VendorOrderReportSearchModel searchModel)
    {
        var bestVendors = (await _orderReportService.GetBestSellingVendorsAsync(null, null, searchModel.Page - 1, searchModel.PageSize));
        var model = await new VendorOrderReportListModel().PrepareToGridAsync(searchModel, bestVendors, () =>
        {
            //fill in model values from the entity
            return bestVendors.SelectAwait(async bestVendor =>
            {
                //fill in model values from the entity
                var bestVendorModel = new VendorOrderReportModel
                {
                    NumberOfOrders = bestVendor.NumberOfOrders,
                    VendorId = bestVendor.VendorId,
                    VendorName = bestVendor.VendorName,
                    Rank = bestVendor.Rank
                };

                //fill in additional values (not existing in the entity)
               // bestVendorModel.TotalAmount = await _priceFormatter.FormatPriceAsync(bestseller.TotalAmount, true, false);

                return bestVendorModel;
            });
        });
    ...
    public async Task<CategoryOrderReportListModel> BestSellingCategories(CategoryOrderReportSearchModel searchModel)
    {
        var bestCategories = await _orderReportService.GetBestSellingCategoriesAsync(null, null, searchModel.Page - 1, searchModel.PageSize);
        var model = await new CategoryOrderReportListModel().PrepareToGridAsync(searchModel, bestCategories, () =>
        {
            //fill in model values from the entity
            return bestCategories.SelectAwait(async bestCategory =>
            {
                //fill in model values from the entity
                var bestCategoryModel = new CategoryOrderReportModel
                {
                    NumberOfOrders = bestCategory.NumberOfOrders,
    ...
    /// The task result contains the bestseller brief search model
    /// </returns>
    public virtual Task<VendorOrderReportSearchModel> PrepareVendorOrderReportSearchModelAsync(VendorOrderReportSearchModel searchModel)
    {
        ArgumentNullException.ThrowIfNull(searchModel);
    ...
    /// The task result contains the bestseller brief search model
    /// </returns>
    public virtual Task<CategoryOrderReportSearchModel> PrepareCategoryOrderReportSearchModelAsync(CategoryOrderReportSearchModel searchModel)
    {
        ArgumentNullException.ThrowIfNull(searchModel);
