...
public partial class CommonModelFactory : ICommonModelFactory
{
    ...
    public virtual async Task<CommonStatisticsModel> PrepareCommonStatisticsModelAsync()
    {
        ...
        model.DeliveryStatisticsModel =  await PrepareDeliveryStatisticsModelAsync();
        model.BusinessStatisticsModel = await PrepareBusinessStatisticsModelAsync();
    ...
    /// <summary>
    /// Prepare Delivery statistics model
    /// </summary>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the Delivery statistics model
    /// </returns>
    public virtual async Task<DeliveryStatisticsModel> PrepareDeliveryStatisticsModelAsync(DeliveryStatisticsSearchModel searchModel = null)
    {
        var model = new DeliveryStatisticsModel();
        model.CompletedOrders = (await _orderService.SearchOrdersAsync())
            .Where(x=> x.OrderStatusId == (int)OrderStatus.Complete).Count();
        model.TodayOrders = (await _orderService.SearchOrdersAsync())
           .Where(x => x.CreatedOnUtc.Date == DateTime.UtcNow.Date).Count();
        model.ReadyToDeliver = (await _orderService.SearchOrdersAsync())
            .Where(x => x.ShippingStatusId == (int)ShippingStatus.NotYetShipped).Count();
        model.PreparingOrders = (await _orderService.SearchOrdersAsync())
            .Where(x => x.OrderStatusId == (int)OrderStatus.Processing).Count();
        model.PendingOrders = (await _orderService.SearchOrdersAsync())
           .Where(x => x.OrderStatusId == (int)OrderStatus.Pending).Count();
        model.OutOfDelivery = (await _orderService.SearchOrdersAsync())
           .Where(x => x.ShippingStatusId == (int)ShippingStatus.Shipped).Count();
        return model;
    }
    ...
    /// <summary>
    /// Prepare Business statistics model
    /// </summary>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the Business statistics model
    /// </returns>
    public virtual async Task<BusinessStatisticsModel> PrepareBusinessStatisticsModelAsync(BusinessStatisticsSearchModel searchModel = null)
    {
        var model = new BusinessStatisticsModel();
        var customerRoleIds = new[] { (await _customerService.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.RegisteredRoleName)).Id };
        model.TotalUsers = (await _customerService.GetAllCustomersAsync(customerRoleIds: customerRoleIds,
            pageIndex: 0, pageSize: 1, getOnlyTotalCount: true)).TotalCount;

        model.ReturnedOrders = (await _returnRequestService.SearchReturnRequestsAsync()).Count();
        

        model.DeliveredOrders = (await _orderService.SearchOrdersAsync())
            .Where(x => x.ShippingStatusId == (int)ShippingStatus.Delivered).Count();


        model.CanceledOrders = (await _orderService.SearchOrdersAsync())
            .Where(x => x.OrderStatusId == (int)OrderStatus.Cancelled).Count();

        model.TotalRevenue = (await _orderService.SearchOrdersAsync()).Where(x=>x.OrderStatusId == (int)OrderStatus.Complete).Sum(order => order.OrderTotal);



        model.TotalVAT = (await _orderService.SearchOrdersAsync()).Where(x => x.OrderStatusId == (int)OrderStatus.Complete).Sum(order => order.OrderTax);
