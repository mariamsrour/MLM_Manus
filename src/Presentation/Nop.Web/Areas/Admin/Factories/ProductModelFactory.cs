...
public partial class ProductModelFactory : IProductModelFactory
{
    ...
    public virtual async Task<ProductModel> PrepareProductModelAsync(ProductModel model, Product product, bool excludeProperties = false)
    {
        ...
        if (product != null)
        {
            ...
            model.AdStatusId = product.AdStatusId;
            model.RejectionReason = product.RejectionReason;
            model.CreatedOnUtc = await _dateTimeHelper.ConvertToUserTimeAsync(product.CreatedOnUtc, DateTimeKind.Local);
        ...
        model.AvailableAdStatus = (await AdStatus.Active.ToSelectListAsync(useLocalization: true)).ToList();
        ...
        model.AvailableCountries = (await _countryService.GetAllCountriesAsync(showHidden: false)).Select(x=> new SelectListItem()
        {
            Text = x.Name,
            Value = x.Name
        }).ToList();

        var country = (await _countryService.GetAllCountriesAsync(showHidden: false)).FirstOrDefault().Id;
        model.AvailableCities = (await _stateProvinceService.GetStateProvincesByCountryIdAsync(country, showHidden:false)).Select(x => new SelectListItem()
        {
            Text = x.Name,
            Value = x.Name
        }).ToList();
        if(product != null && product.Id > 0)
        {
            ...
            {
                //ensure that the first picture is the default one
            ...
            {
                //if no pictures, add a default one
                productPictures = new List<ProductPicture>();
            }
            List<ProductSpecificationAttribute> specificationAttributes = (await _specificationAttributeService
            .GetProductSpecificationAttributesAsync(product.Id)).ToList();
            if(specificationAttributes != null && specificationAttributes.Any())
            {
                var specsModels = specificationAttributes.Select(async (ProductSpecificationAttribute psa) =>
                {
                    var psaModel = psa.ToModel<Nop.Web.Models.Catalog.ProductSpecificationAttributeModel>();
                    psaModel.Id = psa.Id;
                    psaModel.SelectedSpecificationAttributeOptionId = psa.SpecificationAttributeOptionId;
                    var options = await _specificationAttributeService.
                    GetSpecificationAttributeOptionsBySpecificationAttributeAsync(psa.Id);
                    psaModel.Values = options.Select(x => new Nop.Web.Models.Catalog.ProductSpecificationAttributeValueModel
                    {
                        AttributeOptionId = x.Id,
                        ValueRaw = x.Name,
                        Icon = x.Icon,
                ...
                model.ProductSpecsAttributes = (await Task.WhenAll(specsModels)).ToList();
            }
            else
            {
                model.ProductSpecsAttributes = new List<Nop.Web.Models.Catalog.ProductSpecificationAttributeModel>();
            }
           
        }
      

        //prepare product specification attributes
