...
public partial class HomeModelFactory : IHomeModelFactory
{
    ...
    public virtual async Task<DataTablesModel> PrepareBestsellersBriefReportByAmountModelAsync(DataTablesModel model)
    {
        ...
        model.ColumnCollection = new List<ColumnProperty>
        {
            new(nameof(BestsellerModel.Rank))
            {
                Title = "Rank"
            },
    ...
    public virtual async Task<DataTablesModel> PrepareBestsellersBriefReportByQuantityModelAsync(DataTablesModel model)
    {
        ...
        model.ColumnCollection = new List<ColumnProperty>
        {
            ...
            new(nameof(BestsellerModel.ProductName))
            {
             ...
             new(nameof(BestsellerModel.VendorName))
            {
                Title = "Vendor"
            },
            new(nameof(BestsellerModel.TotalQuantity))
    ...
    public virtual async Task<DataTablesModel> PrepareBestSellerVendorsModelAsync(DataTablesModel model)
    {
        ArgumentNullException.ThrowIfNull(model);

        var searchModel = new VendorOrderReportSearchModel();
        searchModel = await _orderModelFactory.PrepareVendorOrderReportSearchModelAsync(searchModel);

        model.Name = "bestseller-vendors-grid";
        model.UrlRead = new DataUrl("BestSellerVendorsList", "Order", new RouteValueDictionary());
        model.Length = searchModel.PageSize;
        model.Dom = "<'row'<'col-md-12't>>" +
                    "<'row margin-t-5'" +
                    "<'col-lg-10 col-xs-12'<'float-lg-left'p>>" +
    ...
    public virtual async Task<DataTablesModel> PrepareBestSellerCategoriesModelAsync(DataTablesModel model)
    {
        ArgumentNullException.ThrowIfNull(model);

        var searchModel = new CategoryOrderReportSearchModel();
        searchModel = await _orderModelFactory.PrepareCategoryOrderReportSearchModelAsync(searchModel);

        model.Name = "bestseller-categories-grid";
        model.UrlRead = new DataUrl("BestSellerCategoriesList", "Order", new RouteValueDictionary());
        model.Length = searchModel.PageSize;
        model.Dom = "<'row'<'col-md-12't>>" +
                    "<'row margin-t-5'" +
                    "<'col-lg-10 col-xs-12'<'float-lg-left'p>>" +
        ...
        model.ColumnCollection = new List<ColumnProperty>
    {
        new(nameof(CategoryOrderReportModel.Rank))
        {
            Title = "Rank"
        },
        new(nameof(CategoryOrderReportModel.CategoryName))
        {
            Title = await _localizationService.GetResourceAsync("Admin.Reports.Sales.BestSellers.Category.Fields.Name")
        },
        new(nameof(CategoryOrderReportModel.NumberOfOrders))
        {
            Title = await _localizationService.GetResourceAsync("Admin.Reports.Sales.BestSellers.Category.Fields.NumberOfOrders")
        },
        new(nameof(CategoryOrderReportModel.CategoryId))
        {
            Title = await _localizationService.GetResourceAsync("Admin.Common.View"),
            Width = "80",
            ClassName = NopColumnClassDefaults.Button,
