...
public partial class ProductController : BaseAdminController
{
    ...
    public virtual async Task<IActionResult> Edit(int id)
    {
        ...
        //    return RedirectToAction("List");
        ViewBag.Vendor = await _vendorService.GetVendorByIdAsync(product.VendorId);
    ...
    public virtual async Task<IActionResult> Edit(ProductModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            bool adStatusChanged = false;
            if (product.AdStatusId != model.AdStatusId)
            {
                adStatusChanged = true;
            }
            ...
            var cus = await _customerService.GetCustomerByVendorIdAsync(product.VendorId);
            if (adStatusChanged)
            {
                if(model.AdStatusId == (int)AdStatus.Active)
                {
                   await _newsService.InsertNewsAsync(new NewsItem
                    {
                        Title = await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdApproved.NotificationTitle"),
                        Full = string.Format(await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdApproved.NotificationBody"), product.Id),
                        CreatedOnUtc = DateTime.UtcNow,
                        StartDateUtc = DateTime.UtcNow,
                        EndDateUtc = DateTime.UtcNow.AddDays(7),
                        CustomerId = cus.Id,
                        LanguageId = lang.Id,
                        Short = string.Format(await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdApproved.NotificationBody"), product.Id),
                        Published = true
                    });

                } else if (model.AdStatusId == (int)AdStatus.Rejected)
                {
                    await _newsService.InsertNewsAsync(new NewsItem
                    {
                        Title = await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdDeclined.NotificationTitle"),
                        Full = string.Format(await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdDeclined.NotificationBody"), product.Id),
                        CreatedOnUtc = DateTime.UtcNow,
                        StartDateUtc = DateTime.UtcNow,
                        EndDateUtc = DateTime.UtcNow.AddDays(7),
                        CustomerId = cus.Id,
                        LanguageId = lang.Id,
                        Short = string.Format(await _localizationService.GetResourceAsync("Admin.Catalog.Products.AdDeclined.NotificationBody"), product.Id),
                        Published = true
                    });
                }
                // send notification to vendor about ad status change
                //  await _WorkflowMessageService.SendAdStatusChangedNotificationAsync(cus, product, lang.Id);
