...
public partial class HomeController : BaseAdminController
{
    ...
    public async Task<IActionResult> BestSellingVendorsPartial(DateTime? startDate, DateTime? endDate, int pageIndex = 1, int pageSize = 10)
    {
        var model = new DataTablesModel();

         model = await _homeModelFactory.PrepareBestSellerVendorsModelAsync(model);
        return PartialView("Table", model);
    }

    public async Task<IActionResult> BestSellingCategoriesPartial(DateTime? startDate, DateTime? endDate, int pageIndex = 1, int pageSize = 10)
    {
        var model = new DataTablesModel();

         model = await _homeModelFactory.PrepareBestSellerCategoriesModelAsync(model);
        return PartialView("Table", model);
    }
    #endregion
}