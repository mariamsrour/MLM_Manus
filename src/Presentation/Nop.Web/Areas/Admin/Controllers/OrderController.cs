...
public partial class OrderController : BaseAdminController
{
    ...
    [HttpPost]
    public virtual async Task<IActionResult> BestSellerCategoriesList(CategoryOrderReportSearchModel searchModel)
    {
        if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageOrders))
            return await AccessDeniedDataTablesJson();

        //prepare model
        var model = await _orderModelFactory.BestSellingCategories(searchModel);

        return Json(model);
    }

    [HttpPost]
    public virtual async Task<IActionResult> BestSellerVendorsList(VendorOrderReportSearchModel searchModel)
    {
        if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageOrders))
            return await AccessDeniedDataTablesJson();

        //prepare model
        var model = await _orderModelFactory.BestSellingVendors(searchModel);
